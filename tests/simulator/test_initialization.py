    ...
    def _test_B_is_as_provided_by_user(self, dim, interp_order, **kwargs):
        ...
        for ilvl, level in hier.levels().items():
            ...
            for ip, patch in enumerate(level.patches):
                ...
                if dim == 1:
                    np.testing.assert_allclose(bx, bx_fn(xbx), atol=1e-16, rtol=0)
                    np.testing.assert_allclose(by, by_fn(xby), atol=1e-16, rtol=0)
                    np.testing.assert_allclose(bz, bz_fn(xbz), atol=1e-16, rtol=0)
